...
parser = ArgumentParser(description = "ExhaustiveExploration")
parser.add_argument('instance_id', type=str, help='instance_id')
parser.add_argument('start_permutation', type=int, help='initial permutation')
...
stdout = sys.stdout
outfilename = f'{args.instance_id}-{args.start_permutation}.stdout'
with open(outfilename, 'w') as sys.stdout:
    df = runner.run_once("ExhaustiveExploration", args.instance_name, args.start_permutation, budget = budget, out_filename = args.output)
